...
import useCreatePassword from "../../../hooks/serverQueries/passwords_hooks/useCreatePassword";
// ----------------------------------------------------------------------
...
export default function PasswordNewForm() {
  ...
  //   await createPassword.mutate(values.appName, values.login, values.password);
  // };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      try {
        ...
        //        resetForm();
        if (createPassword.isSuccess) router.push("/passwords");
      } catch (error) {
        console.error(error);
        setSubmitting(false);
        setErrors(error);
      ...
      {createPassword.isError ? (
        <Stack sx={{ my: 1 }} spacing={3}>
          <Alert severity="error">
            {createPassword.error?.response?.data?.msg}
            {createPassword.error?.response?.data?.errors?.map((item) => (
              <>
                -{item.msg}
                <br />
              </>
